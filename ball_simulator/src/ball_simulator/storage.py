...
class HDF5TrajectoryWriter:
    ...
    def write(self, index: int, trajectory: Trajectory) -> None:
        if self._file is None:
            raise RuntimeError("Writer must be used as a context manager")
        group = self._file.create_group(f"trajectories/{index:08d}")
        self._write_arrays(group.create_group("observations"), trajectory.observations)
        if trajectory.high_rate is not None:
            self._write_arrays(group.create_group("high_rate"), trajectory.high_rate)
        params = group.create_group("parameters")
        for name in trajectory.parameters.__dataclass_fields__:
            params.attrs[name] = getattr(trajectory.parameters, name)
